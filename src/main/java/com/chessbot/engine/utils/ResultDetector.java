...
public final class ResultDetector {
    ...
    public static boolean isInsufficientMaterial(Board board) {
        ...
        // There is an insufficient material edge case where if there are only 1 or more bishops for each player, if they
        // are all on same colored squares, it's a draw. Checks if there are no knights (so there are only bishops) and
        // if all bishops are on dark or light squares (so all bishops are on same colored squares)
