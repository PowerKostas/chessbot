...
public final class MoveOrderer {
    ...
    // Goes through every legal move and adds all the different move ordering heuristic scores to the last 16 bits of the
    // Move int
    ...
    // Starting from the currently searched index of the given move list, it finds the best move based on the scores assigned
    // from the move ordering heuristics. Then the best move is placed at the currently searched index
    public static void selectNextMove(MoveList moveList, int currentIndex) {
        int maxMove = moveList.moves[currentIndex];
        int maxIndex = currentIndex;

        // Because the score is in the upper bits of the Move int, it doesn't have to be extracted, the whole object can
        // just be compared
