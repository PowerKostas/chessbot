...
public final class Attacks {
    ...
    public static long generateMap(Board board, int color) {
        ...
        // Filters out the friendly king in the calculations. This is done in order to avoid the king being able to move
        // backwards in the same direction as a checking slider when filtering the attack map to legal moves
        ...
        while (rooksQueensBitboards != 0L) {
            int rookSquare = Long.numberOfTrailingZeros(rooksQueensBitboards);
            attackMapBitboard |= Rook.attacks(rookSquare, allPiecesBitboard);
            rooksQueensBitboards ^= (1L << rookSquare);
        }
        ...
        while (bishopsQueensBitboards != 0L) {
            int bishopSquare = Long.numberOfTrailingZeros(bishopsQueensBitboards);
            attackMapBitboard |= Bishop.attacks(bishopSquare, allPiecesBitboard);
            bishopsQueensBitboards ^= (1L << bishopSquare);
        }
